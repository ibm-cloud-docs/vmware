...
4. Select the servers. See VMware Support publications for minimum requirements:
  * The following servers were selected for the example. **Note:** Having dual unbonded public and private uplinks is a requirement for redundancy. Confirm that the data center where you created your VLANs are unbonded uplinks.  
...
2. Click **Account > Place an Order**.
3. Order either a **Monthly** Bare Metal or Virtual Server public or private node Instance (VSI) under Virtual Servers.
  1. For an IBM Cloud Virtual Server Instance (VSI) to qualify for vCenter 6.0, you must deploy on at least **4 x 2.0 GHz Cores** and **4 GB of RAM**
...
5. If this a new IBM Cloud Data Center deployment, proceed to step 6. If this deployment is an expansion or deployment in to a existing Data Center, select the preferred Backend BCR VLAN + Frontend FCR VLAN. For our example, the Backend BCR VLAN is 1101 and the Frontend FCR VLAN is 2200. (If this is a new deployment or a new deployment into a new DC, the Backend BCR and Frontend FCR VLANs are created when you order the server)
6. Enter a **Host name** and **Domain**.
7. Review the order and click **Finalize Your Order** to start the provisioning process.
...
17. Right-click the new cluster that you created and select **Add Host**.
18. Enter the IP address or Host name of one of the vSphere hosts and enter **Root** in the **User name** field and the password for the host in the **Password** field.
...
2. Under **Hardware** section, select **Networking**. Make sure that you have vSwitch0 with vmnic0; click **Properties** next to vSwitch0.
...
A port group now needs to be added for vMotion. The group can be created as a Virtual Standard Switch (VSS) or Virtual Distributed Switch (VDS). For purpose of our example, create a VSS switch.
...
5. Complete the Port Group Properties with the following information:
...
6. Click **Next**.
7. Enter a **Portable IP address** for the VLAN. (You can get the port IP address from the IBM Cloud portal, **Network > IP Management > VLANs**. Select the correct VLAN and under **Subnets** you see a portable IP address range. If you have no available Portable IP address or if you have exhausted your current pool, follow the steps in the section 'Order Private Subnets / IP addresses ' to order extra Portable IP address
...
2. Complete the **Port Group Properties** with the following information:
  * **Network Label:** Enter the name of the Port Group (for example, VM Data).
