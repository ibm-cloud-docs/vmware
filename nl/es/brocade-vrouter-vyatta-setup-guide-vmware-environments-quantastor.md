...
# Configuración de Brocade vRouter (Vyatta) para entornos de VMware con QuantaStor

Puede configurar un dispositivo Brocade vRouter (Vyatta) [configuración de alta disponibilidad (HA)] en un entorno de VMware que utiliza QuantaStor. Utilice la información siguiente con la [Arquitectura avanzada de referencia de VMware de un solo sitio](/docs/infrastructure/virtualization/advanced-single-site-vmware-reference-architecturesoftlayer.html) para establecer una de estas opciones de almacenamiento en el entorno de VMware.
{:shortdesc}
...
En la configuración de ejemplo, hay cinco zonas que se crean en Brocade vRouter (Vyatta):

* SLSERVICE: servicios de {{site.data.keyword.BluSoftlayer_full}}
...
La figura 1 describe la comunicación entre cada zona. **Nota:** Su entorno puede ser diferente y puede necesitar diferentes zonas y reglas de cortafuegos.

![Figura 1: Configuración de zonas de Brocade vRouter (Vyatta)](images/brocade_figure1.png "Configuración de zonas de Brocade vRouter (Vyatta)") Figura 1. Configuración de zonas de Brocade vRouter (Vyatta)

## Configuración de Brocade vRouter (Vyatta)

Para configurar Brocade vRouter (Vyatta), siga estos pasos:
...
Configure las interfaces de enlace en los Brocade vRouters (Vyatta) que va a vincular a las subredes en el entorno. Debe sustituir las VLAN (1101, 1102 y 1103) con las VLAN correspondientes de su entorno. Además, tenga en cuenta que las instrucciones incluidas con '<>' se deben sustituir con los detalles de su entorno (sin '<>').

Utilice los mandatos siguientes para configurar las interfaces de enlace en los Brocade vRouters (Vyatta). Debe estar en la modalidad `configure`.
...
Configure SNAT de forma que las VM y las VM de gestión del clúster de capacidad puedan acceder a Internet. A partir de este paso, la configuración debe realizarse en un solo Brocade vRouter (Vyatta), ya que la sincronización de la configuración se produce más tarde.

Utilice los mandatos siguientes en modalidad `configure`:
...
A continuación, deberá configurar los grupos de cortafuegos que están asociados con determinados rangos de IP.
...
En este paso, va a enlazar zonas concretas con interfaces en el Brocade vRouter (Vyatta).
...
Ahora va a aplicar las reglas de cortafuegos a la comunicación entre zonas.
...
Puesto que ha configurado uno de los Brocade vRouters (Vyatta) del par de alta disponibilidad, debe sincronizar los cambios en el con el otro dispositivo de pasarela.
...
1. Inicie sesión en el portal de clientes de IBM Cloud, pulse **Red, Dispositivo de pasarela** y pulse el Brocade vRouter (Vyatta).
2. Seleccione una VLAN y pulse **Asociar**.
3. Repita el paso 2 para cada VLAN que haya creado para su entorno. Las VLAN deben tener la opción de direccionamiento habilitada para asociarse con el Brocade vRouter (Vyatta).
...
Las VLAN están ahora direccionadas a través del Brocade vRouter (Vyatta). Si observa que la comunicación está obstaculizada entre dos zonas, omita una o varias de las VLAN en cuestión y compruebe los valores del Brocade vRouter (Vyatta).

Ahora tiene en funcionamiento un entorno de VMware de un solo sitio que está protegido por un Brocade vRouter (Vyatta) en IBM Cloud.
